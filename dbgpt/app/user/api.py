...
from dbgpt.app.user.schemas import UserAuth, TokenSchema, UserOut
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if not verify_password(form_data.password, hashed_pass):
        Result.failed(code="E000X", msg=f"Incorrect email or password")
    token = create_access_token(user.data[0].email)
    refresh_token = create_refresh_token(user.data[0].email)
    tokenSchema = TokenSchema(access_token=token , refresh_token=refresh_token)
    return Result.succ(tokenSchema)
